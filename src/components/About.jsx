  ...
  return (
    <div className="min-h-screen h-screen border-x-black flex flex-col p-5">
      ...
      <div className="md:w-2/3 flex md:flex-row flex-col h-full">
        <div className="flex flex-col md:h-full h-auto md:justify-around sm:gap-0 gap-2">
          {headings.map((heading, key) => (
            <div key={key}>
              <p className="lg:text-3xl md:text-2xl text-xl text-red-600 font-mono text-center">
                {heading}
              </p>
              <p className="text-justify lg:text-lg sm:text-base text-sm font-serif">{content[key]}</p>
            </div>
          ))}
        </div>
      </div>
